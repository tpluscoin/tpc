...
uint256 hashGenesisBlock("0x00000345c52afbb1f514c70ba861e0d892aff2f83f104128fee4f201a5b86c7b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TPlusCoin: starting difficulty is 1 / 2^12
static CBigNum bnInitialDifficulty(~uint256(0) >> 20);
...
const string strMessageMagic = "TPlusCoin Signed Message:\n";
...
{
    // TPlusCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // TPlusCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
// Genesis main net
		...
		const char* pszTimestamp = "tplus genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1503331666;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1142179;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0x9a }; // TPlusCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TPlusCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TPlusCoinMiner : ProcessBlock, block not accepted");
    }
